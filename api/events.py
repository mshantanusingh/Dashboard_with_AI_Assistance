...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(__file__)), "mcp-servers", "events"))
...
@app.get("/api/events/mcp/tools")
def discover_tools():
    return {"server": "events", "version": "1.0.0", "tools": get_all_tool_schemas()}

@app.post("/api/events/mcp/execute")
def execute_mcp_tool(request: ToolExecutionRequest):
    result = execute_tool(request.tool, request.parameters)
    return ToolExecutionResponse(success=result["success"], result=result["result"], error=result.get("error"), server="events")
